...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($row10["result"] == "true") {
        ...
        $sql = "INSERT INTO student (sname, fname, roll_no, dob, branch, semester, email_id, room_no, s_pnumber, f_pnumber, street_address, pin_code, city, state, gender, password)
            VALUES ('$sname','$fname','$rollno','$dob','$branch','$semester','$emailID','$roomNumber','$spnumber','$fpnumber','$address','$pinCode','$city','$state','$gender','$pass');";

        mysqli_query($conn, $sql);

        $sql = "SELECT C_ID FROM course WHERE semester=?;";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "s", $param_semester);
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                mysqli_stmt_bind_param($stmt2, "s", $param_c_id);
                $param_c_id = $cid;
                ...
                if (mysqli_num_rows($result2) > 0) {
                    ...
                    $fid = $row2['f_id'];
                    $sql3 = "INSERT INTO study_from VALUES('$rollno', '$fid', '$cid');";
                    mysqli_query($conn, $sql3);

                    $sql3 = "INSERT INTO takes VALUES('$rollno', '$cid');";
                    mysqli_query($conn, $sql3);
                }
            }
        }
